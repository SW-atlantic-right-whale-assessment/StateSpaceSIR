...
summary_table <- function( SIR, file_name = NULL){
    ...
    # Vars of interest
    years <- sort(unique(c(SIR$inputs$target.Yr,SIR$inputs$output.Years)))
    ...
    pop_vars <- c("K", "Nmin", paste0("N_", years))
    depletion_vars <- c("Max_Dep", paste0("status", years),"P50","var_N", paste0("q_IA1", 1:num.IA), paste0("q_IA2", 1:num.IA), "add_VAR_IA")

    results <- data.frame(matrix(NA, nrow = length(c(output_vars, trajectory_vars)), ncol = 8))
    colnames(results) <- c("Parameter","Mean", "Median", "2.5% CI", "25% CI", "75% CI", "97.5% CI", "Unique")

    x <- SIR$resamples_output[,output_vars]
    x$var_N <- sqrt(x$var_N)

    traj <- SIR$resamples_trajectories[,paste0("N_", years)]
    depletion <- traj / SIR$resamples_output$K
    colnames(depletion) <- paste0("status", years)

    x<- cbind(x, traj, depletion)
    ...
    # Format things
    vars <- c(output_vars, trajectory_vars)
